...
// TODO
// This kernel takes in an input channel as a uchar and blurs it
// according to a square filter of provided weights
__global__
                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  // make sure not to go outside the bounds of the image, and if threads
  // are mapped there, return early
                      ...
                      unsigned char* const blueChannel)
{
  ...
  // we need to get the index of the uchar4 we're grabbing, and put their
  // contents in proper channels.
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize((numCols/20)+1, (numRows/20)+1, 1);
  ...
  // Call my convolution kernel here 3 times, once for each color channel.
  // using d_filter instead of h_filter to take advantage of mem access speeds
  gaussian_blur<<<gridSize,blockSize,filterWidth*filterWidth*sizeof(float)>>>(d_red, d_redBlurred, numRows, numCols, d_filter, filterWidth);
  gaussian_blur<<<gridSize,blockSize,filterWidth*filterWidth*sizeof(float)>>>(d_green, d_greenBlurred, numRows, numCols, d_filter, filterWidth);
  gaussian_blur<<<gridSize,blockSize,filterWidth*filterWidth*sizeof(float)>>>(d_blue, d_blueBlurred, numRows, numCols, d_filter, filterWidth);
